...
namespace ngrokAspNet
{
  public class TunnelService : BackgroundService
  {
    ...
    {
      var twilioClient = new TwilioRestClient(_configuration["TwilioAccountSid"], _configuration["TwilioAuthToken"]);
      var phoneNumber = (await IncomingPhoneNumberResource.ReadAsync(
        phoneNumber: new Twilio.Types.PhoneNumber(_configuration["TwilioPhoneNumber"]),
        limit: 1,
        client: twilioClient))
        .Single();
